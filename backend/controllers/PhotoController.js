...
const getUserPhotos = async (req, res) => {
    try {
        ...
        // Retorna as fotos após aguardar a resolução da promessa
        return res.status(200).json(photos);
    } catch (error) {
...
const getPhotoById = async (req, res) => {
  ...
  try {
      ...
      console.error(error);
      return res.status(500).json({ errors: ["Erro interno do servidor", error.message] });
  }
};
...
const commentPhoto = async (req, res) => {
    ...
    if (!comment.trim()) {
      res.status(422).json({ errors: ["O comentário não pode estar vazio."] });
      return;
    }
